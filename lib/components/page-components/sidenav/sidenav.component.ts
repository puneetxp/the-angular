...
export class SidenavComponent implements OnInit {
  ...
  constructor() {
    ...
    this.isExtraScreenSmall = this.breakpoints.observe(
      `(max-width: ${EXTRA_SMALL_WIDTH_BREAKPOINT}px)`,
    )
      .pipe(map((breakpoint) => breakpoint.matches));
    this.isScreenSmall = this.breakpoints.observe(
      `(max-width: ${SMALL_WIDTH_BREAKPOINT}px)`,
    )
      .pipe(map((breakpoint) => breakpoint.matches));
  }
  ngOnInit() {
    this.applyMenusFromRoute(this._route.snapshot.data?.["menus"]);
    this.subscriptions.add(
      this._route.data.subscribe((data) =>
        this.applyMenusFromRoute(data?.["menus"])
      ),
    );
    this.title();
    ...
    this.subscriptions.add(
      this.params.subscribe((latest) => this.currentParams = latest),
    );
    this.subscriptions.add(
      this._navigationFocusService.navigationEndEvents.pipe(
  ...
  title(): void {
    ...
    const active = this.menus.find((i) =>
      this.router.url == i.route ||
      this.router.url.match(
        new RegExp(this.normalizeRoute(i.route) + "$", "g"),
      ) ||
      i.child &&
        i.child.find((child) =>
          this.router.url == child.route ||
          this.router.url.match(
            new RegExp(this.normalizeRoute(child.route) + "$", "g"),
          )
        )
    );
  ...
  private syncMenuBarActions(menu?: Menu) {
    ...
    (menu.child || []).forEach((child) => {
      if (
        ...
        this.editingContext = contextValue
          ? `${child.label || child.name} (${contextValue})`
          : (child.label || child.name);
      }
    });
    const explicitActions: (Menu | MenuAction)[] =
      Array.isArray(menu.actions) && menu.actions.length > 0
        ? [...menu.actions]
        : [
          ...((menu.child || []).filter((child) =>
            child.route && !child.route.includes("/:")
          )),
          ...(menu.links || []),
        ];
    ...
    const primaryCandidate = buttons.find((btn) => btn.icon === "add") ||
      buttons[0];
    this.menuBarService.setPrimaryActionButton(primaryCandidate);
    buttons.filter((btn) => btn !== primaryCandidate).forEach((btn) =>
      this.menuBarService.setActionButton(btn)
    );
  }
  private toActionButton(
    menu: Menu | MenuAction,
  ): MenuActionButton | undefined {
  ...
  private applyMenusFromRoute(source?: Menu[]) {
    if (!source || !source.length) {
      return;
    }
    const hydrated = [...source];
    const hasBackLink = hydrated.some((menu) => menu.route === "/back");
    if (!hasBackLink) {
      hydrated.push({
