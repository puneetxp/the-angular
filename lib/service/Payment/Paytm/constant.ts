const CONSTANTS = {
  ...
  ERRORS: {
    ...
    CHECKOUT_NOT_AVAILABLE:
      "Checkout JS library not found. Please make sure you have included checkout js!",
...
Object.keys(CONSTANTS.ERRORS).forEach((key) => {
  const errorCode = key as keyof typeof CONSTANTS.ERRORS;
  if (
    [
      "INIT",
      "INVOKE",
      "MERCHANT_ID_NOT_FOUND",
      "INVALID_CHECKOUT_JS_INSTANCE",
      "CHECKOUT_NOT_AVAILABLE",
    ].includes(errorCode)
  ) {
    CONSTANTS.ERRORS[errorCode] = `${CONSTANTS.PROJECT_NAME}: ${
      CONSTANTS.ERRORS[errorCode]
    }`;
  }
});
